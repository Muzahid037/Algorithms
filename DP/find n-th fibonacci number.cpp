...
{
    if(n==0 || n==1) return 1;
    if(storefibo[n]==0) return storefibo[n]=fibo(n-1)+fibo(n-2);
    else return storefibo[n];
}
...
void solve()
{
    ...
    cin>>n;
    cout<<fibo(n-1)<<endl;
    ///passing (n-1) because fibonacci no starts with 0-based'th,but here we want 1-based'th no
}
int main()
{
    Boost();
